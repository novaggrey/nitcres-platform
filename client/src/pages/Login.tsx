...
export default function Login() {
  ...
  const enterDemo = () => {
    ...
    demoMutation.mutate({ email, password });
  };
        ...
        <section className="flex items-center justify-center px-6 py-10 sm:px-10 lg:px-16">
          <div className="w-full max-w-md">
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-5 shadow-2xl shadow-black/20">
              ...
              {demoMutation.isError && <p role="alert" className="mt-4 rounded-xl border border-rose-300/20 bg-rose-300/[0.06] px-3 py-2 text-xs leading-5 text-rose-200">{demoMutation.error?.message ?? "Demo access could not be validated. Please try again."}</p>}
              <Button onClick={enterDemo} disabled={demoMutation.isPending || !email.trim() || !password} className="mt-5 h-11 w-full rounded-xl bg-cyan-300 font-semibold text-[#06111d] transition duration-200 hover:-translate-y-0.5 hover:bg-cyan-200 active:translate-y-0">{demoMutation.isPending ? "Validating secure demo access…" : `Enter ${demoRoles.find((item) => item.key === selectedRole)?.label ?? "demo"} workspace`} <ArrowRight size={16} className="ml-2" /></Button>
